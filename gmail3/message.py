...
class Message():
    ...
    def __str__(self) -> str:
        excludes = ["html", "headers", "message", "mailbox", "gmail"]
        buffer = []
        for k, v in self.__dict__.items():
            if k not in excludes:
                buffer.append("{:<15} {}".format(k, v))
        return "\n".join(buffer)
    ...
    def parse_labels(self, headers):
        if re.search(r'X-GM-LABELS \(([^\)]+)\)', headers):
            ...
            return [
                l.replace('"', '').decode("unicode_escape") if isinstance(l, bytes) else
                l.replace('"', '')
                for l in labels
            ]
        else:
    ...
    def parse_subject(self, encoded_subject):
        ...
        elif isinstance(encoded_subject, str):
